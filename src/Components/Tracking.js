...
const Tracking = () => {
  ...
  const chartTypeOptions = ["BarChart", "Pie"];
  ...
  useEffect(() => {
    const fetchSessionData = async () => {
      try {
        ...
        if (
          showMonth &&
          helpers.checkIsValidMonth(showMonth) == new Date().getMonth + 1
        ) {
          setCharData();
        }
      } catch (e) {
    ...
    const fetchChartData = async () => {
      try {
        const response = await axiosInstance.post(
          `/api/budget-data?month=${showMonth}`
        );
  ...
  const handleShowMonthChange = (event) => {
    if (
      helpers.checkIsValidMonth(event.target.value) >
      new Date().getMonth() + 1
    ) {
    ...
    if (!incomeMonth || !updatedIncomeDescription || !updatedIncome) {
      ...
      try {
        ...
        if (
          ...
          return alert(
            "You need to create a budget(saving goal) for this month before updating income."
          );
        }
        setUpdatedIncome("");
        setIncomeMonth("");
        ...
        return alert(
          "Update Income Successfully, you have been awarded 2 points!"
        );
      } catch (error) {
        console.log(error);
      }
  ...
  if (token) {
      ...
      return (
        <div className="container">
          {budgetData ? (
            <div className="info-bar">
              ...
              <h2>
                Estimated Income:{" "}
                {budgetData &&
                budgetData.income[budgetData.income.length - 1].amount
                  ? budgetData.income[budgetData.income.length - 1].amount
                  : "$3000"}
              </h2>
              <h2>
          ...
          <div className="trackingForm-container">
            <div className="set-goal">
              <Tooltip
                title="You can set your goal for current or next month in this year. 
        You are allowed to reset goal for current or future month. But you cannot reset goal or try to update estimated income for past month here."
            ...
            <div className="update-income">
              <form>
                <Tooltip
                  title="If you find your actual income is lower or higher that your estimated income after that month, you can update it here.
          ...
          <div className="chart-container">
            <div className="expense-chart">
              ...
              <InputLabel id="chartType-selection">
                Select Chart Type
              </InputLabel>
              <Select
              ...
              {charData ? (
                <ChartComponent
                  ...
                  chartData={parseChartData(charData)}
                  title={showMonth}
                />
              ) : (
            ...
            <div className="saving-chart">
              ...
              <ChartComponent
                chartType="BarChart"
                chartData={parseSummaryData(summaryData, "Monthly Saving")}
                title="Monthly Saving"
              />
              <ChartComponent
                chartType="BarChart"
                chartData={parseSummaryData(summaryData, "Monthly Income")}
                title="Monthly Income"
              />
              <ChartComponent
                chartType="line"
                chartData={parseSummaryData(summaryData, "Monthly Expense")}
    ...
    return (
      <Navigate
        to="/auth/login"
        state={{ otherError: "Please login before access your tracking page." }}
      />
    );
  }
};
